    ...
    private fun parseRoute(route: JSONObject): NavRoute {
        ...
        if (stepsJson != null) {
            for (i in 0 until stepsJson.length()) {
                ...
                // Per-step polylines carry the full road geometry, unlike the
                // simplified overview polyline that can cut corners. Skip
                // duplicated boundary points where consecutive steps meet.
                val poly = s.optString("polyline")
                if (poly.isNotBlank()) {
                    decodePolyline(poly).forEach { p ->
                        if (stepPoints.isEmpty() || stepPoints.last() != p) stepPoints.add(p)
                    }
                }
            }
        }

        // Prefer the detailed step geometry so the line sits exactly on the
        // roads; fall back to the simplified overview polyline.
        val points = if (stepPoints.size >= 2) stepPoints else decodePolyline(overview)
